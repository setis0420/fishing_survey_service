...
export default function VoyageInquiry() {
  ...
  const [availableMonths, setAvailableMonths] = useState<TrackMonth[]>([])
  const [selectedMonth, setSelectedMonth] = useState<TrackMonth | null>(null)

  // 항적 HTML
  const [trackHtml, setTrackHtml] = useState<string>('')

  // 월별 항차 및 데이터
  const [monthlyVoyage, setMonthlyVoyage] = useState<VoyageData | null>(null)
  ...
  // 다이얼로그 상태
  const [addAuctionDialogOpen, setAddAuctionDialogOpen] = useState(false)
  ...
  const handleSelectVessel = async (vessel: VesselRegistry) => {
    ...
    if (vessel.mmsi) {
      setLoading(true)
      try {
        const res = await getTrackYears(vessel.mmsi)
        setAvailableYears(res.years)
      } catch (error) {
        console.error('연도 조회 실패:', error)
        setAvailableYears([])
      } finally {
        setLoading(false)
      }
  ...
  const handleSelectYear = async (year: number) => {
    ...
    if (selectedVessel?.mmsi) {
      setLoading(true)
      try {
        const res = await getTrackMonths(selectedVessel.mmsi, year)
        setAvailableMonths(res.months)
      } catch (error) {
        console.error('월 조회 실패:', error)
        setAvailableMonths([])
      } finally {
        setLoading(false)
      }
  ...
  const handleSelectMonth = async (month: TrackMonth) => {
    setSelectedMonth(month)

    if (selectedVessel?.mmsi && selectedYear) {
      setLoading(true)
      try {
        // 항적 HTML 로드
        const htmlRes = await getTrackHtml(selectedVessel.mmsi, month.filename)
        setTrackHtml(htmlRes.html)

        // 월별 항차 조회/생성
        const voyageRes = await getOrCreateMonthlyVoyage(
          selectedVessel.mmsi,
          selectedYear,
          month.month,
          selectedVessel.vessel_name
        )
        setMonthlyVoyage(voyageRes.data)

        // 위판/사매/경비 데이터 로드
        const [auctionsRes, privateSalesRes, expensesRes] = await Promise.all([
          getAuctions(voyageRes.data.id),
          getPrivateSales(voyageRes.data.id),
          getExpenses(voyageRes.data.id),
        ])
  ...
  const openAddAuctionDialog = () => {
    ...
    setAddAuctionDialogOpen(true)
  }

  const handleAddAuction = async () => {
    if (!monthlyVoyage) return
    setLoading(true)
    try {
      await createAuction({
        voyage_id: monthlyVoyage.id,
        ...auctionForm,
      })
      const res = await getAuctions(monthlyVoyage.id)
      setAuctions(res.data)
      setAddAuctionDialogOpen(false)
    } catch (error) {
  ...
  const handleAddPrivateSale = async () => {
    ...
    try {
      ...
      const res = await getPrivateSales(monthlyVoyage.id)
  ...
  const handleAddExpense = async () => {
    ...
    try {
      ...
      const res = await getExpenses(monthlyVoyage.id)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-foreground">항적/어획 조회,수정</h2>
          <p className="text-sm text-muted-foreground">어선 선택 → 연도 선택 → 월 선택 → 항적/위판/사매/경비 조회</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <ChevronRight className="h-4 w-4 text-muted-foreground" />
        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${
          selectedMonth ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
        }`}>
        ...
        <ChevronRight className="h-4 w-4 text-muted-foreground" />
        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${
          monthlyVoyage ? 'bg-green-600 text-white' : 'bg-muted text-muted-foreground'
        }`}>
        ...
        <Card className="lg:col-span-3 shadow-sm">
          <CardHeader className="pb-3">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-base">연도 / 월 선택</CardTitle>
            </div>
            <CardDescription>
                ...
                <div className="space-y-2">
                  ...
                  {availableYears.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground py-2">
                      {selectedVessel.mmsi ? '사용 가능한 항적 데이터가 없습니다' : 'MMSI가 없는 어선입니다'}
                    </p>
                  )}
                </div>
                ...
                {selectedYear && (
                  <div className="space-y-2">
                    <Label className="text-xs text-muted-foreground">
                      월 선택 ({availableMonths.length}개월)
                    </Label>
                    <div className="grid grid-cols-3 gap-2 max-h-[280px] overflow-auto pr-1">
                      {availableMonths.map((m) => (
                        ...
                        >
                          <div className="text-lg font-bold">{m.month}월</div>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-base">항적/어획 정보</CardTitle>
              </div>
              {selectedMonth && monthlyVoyage && (
            ...
            {selectedMonth && (
              <CardDescription>
                {selectedVessel?.vessel_name} | {selectedYear}년 {selectedMonth.month}월 |
                수신건수: {selectedMonth.count.toLocaleString()}건
              </CardDescription>
            )}
          </CardHeader>
          <CardContent>
            {monthlyVoyage ? (
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                <TabsContent value="track" className="mt-4">
                  {trackHtml ? (
                    <div className="rounded-lg border overflow-hidden" style={{ height: '450px' }}>
                      <iframe
                        srcDoc={trackHtml}
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-muted-foreground" style={{ height: '450px' }}>
                      <Navigation className="h-12 w-12 mb-3 opacity-30" />
                      <p className="text-sm font-medium">항적 데이터 로딩 중...</p>
                    </div>
                  )}
                </TabsContent>
                ...
                <TabsContent value="auction" className="mt-4">
                  <div className="flex justify-end mb-3">
                    <Button onClick={openAddAuctionDialog} size="sm" className="gap-1.5">
                      <Plus className="h-4 w-4" />
                      위판 등록
                    </Button>
                  </div>
                  <div className="rounded-lg border overflow-hidden" style={{ maxHeight: '350px', overflowY: 'auto' }}>
                    <Table>
                      <TableHeader>
                      ...
                      <TableBody>
                        {privateSales.map((sale) => (
                          <TableRow key={sale.id} className="hover:bg-muted/30">
                            ...
                            <TableCell className="text-sm text-muted-foreground max-w-[150px] truncate" title={sale.note || ''}>
                              {sale.note || '-'}
                            </TableCell>
                            <TableCell>
                              <Button
              ...
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground" style={{ height: '500px' }}>
                <Navigation className="h-12 w-12 mb-3 opacity-30" />
                <p className="text-sm font-medium">데이터 조회 대기 중</p>
                <p className="text-xs mt-1">어선 → 연도 → 월을 순서대로 선택해주세요</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {/* 위판 등록 다이얼로그 */}
      <Dialog open={addAuctionDialogOpen} onOpenChange={setAddAuctionDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {' | '}
              <span className="font-mono">{monthlyVoyage?.id}</span>
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              <Label htmlFor="auction_note">메모 (선택)</Label>
              <Input
                id="auction_note"
                placeholder="비고 또는 메모 입력..."
                value={auctionForm.note}
                onChange={(e) => setAuctionForm({ ...auctionForm, note: e.target.value })}
              />
            </div>
            {/* 예상 금액 미리보기 */}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddAuctionDialogOpen(false)}>취소</Button>
            <Button
